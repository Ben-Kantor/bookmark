...
export const updateExplorerState = (activePath: string): void => {
  ...
  if (!newActiveLink && activePath.endsWith(".md")) {
    ...
    newActiveLink = document.querySelector(
      `a[href="${encodeURI(pathWithoutMd)}"]`,
    ) as HTMLElement;
    if (!newActiveLink)
      newActiveLink = document.querySelector(
        `a[href="${pathWithoutMd}"]`,
      ) as HTMLElement;
  }
...
export function initResizablePanel(
  ...
  const startResize = (e: MouseEvent) => {
    ...
    const doResize = (moveE: MouseEvent) => {
      if (isClick && Math.abs(moveE.clientX - startX) > 5) {
        isClick = false;
        isDragging = true;
        // Disable iframe pointer events only when dragging starts
      ...
      const deltaX =
        direction === "left" ? moveE.clientX - startX : startX - moveE.clientX;
    ...
    const stopResize = () => {
      ...
      // Restore iframe pointer events when dragging stops
      const iframes = document.querySelectorAll("iframe");
      iframes.forEach((iframe) => {
      ...
      if (isClick) togglePanel();
        ...
        if (finalWidth < 60) panel.classList.add("panel-collapsed");
      }
    };
