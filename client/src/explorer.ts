...
const createExplorerStructure = (
  ...
  return visibleItems
    ...
    .map((item) => ({
      ...
      isActive: !item.dir &&
        decodeURIComponent(`${pathPrefix}${item.name}`) === activePath,
      isExpanded: item.dir
        ? (
...
const generateDirectoryHTML = (item: ExplorerItem): string => {
  ...
  const childrenHTML = item.isExpanded && item.children.length > 0
    ? `<ul id="${folderId}" role="group">${
      generateExplorerHTML(item.children)
    }</ul>`
    : `<ul id="${folderId}" role="group" style="display: none">${
      generateExplorerHTML(item.children)
    }</ul>`;
...
const generateFileHTML = (item: ExplorerItem): string => {
  const iconChar = fileTypeIcons[item.extension?.toLowerCase() || ""] ??
  ...
  const liClasses = item.isActive
    ? "rounded p-0 active-file-row"
    : "rounded p-0";

  return `
    <li class="${liClasses}" role="treeitem">
      <a href="${encodeURI(item.path)}"
         class="flex items-center w-full hover:text-[var(--text-accent)]${
    iconChar ? " gap-2" : ""
  }${activeClasses}"
         style="padding-left: 0"
         ${ariaCurrent}>
...
export const renderExplorer = (
  ...
  const explorerStructure = createExplorerStructure(
    items,
    pathPrefix,
    activePath,
  );
...
export function initResizablePanel(
  ...
  handle.onmousedown = (e: MouseEvent) => {
    ...
    const onMouseMove = (moveE: MouseEvent) => {
      ...
      const deltaX: number = direction === "left"
    ...
    const onMouseUp = () => {
      ...
      if (isClick) {
        const isCollapsed: boolean = panel.classList.contains(
          "panel-collapsed",
        );
