import { expandedFolders, FOLDER_CLOSED_ICON, FOLDER_OPEN_ICON } from './constants.ts'
...
const setupExplorerEventDelegation = (explorerList: HTMLUListElement): void => {
	explorerList.addEventListener('click', (e: MouseEvent) => {
		...
		if (hasChildren)
			iconSpan.textContent = isNowExpanded ? FOLDER_OPEN_ICON : FOLDER_CLOSED_ICON
	})
}
...
export function initResizablePanel(
	...
	const startResize = (e: MouseEvent) => {
		...
		const doResize = (moveE: MouseEvent) => {
			...
			const deltaX = direction === 'left' ? moveE.clientX - startX : startX - moveE.clientX
