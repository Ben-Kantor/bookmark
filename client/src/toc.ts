...
import { expandedTocItems, mainContentEl } from './constants.ts'
...
export const generateToc = (): void => {
	...
	initResizablePanel('toc-panel', 'toc-handle', 'right')
}
...
function renderTocDir(
	...
	const icon: string = isExpanded ? headingOpen : headingClosed

	const button: HTMLButtonElement = document.createElement('button')
	button.className = 'toc-folder-button'
	button.style.paddingLeft = '0'
	button.innerHTML = `<span class="item-icon icon" aria-hidden="true">${icon || ''}</span>` +
	...
	button.setAttribute(
		'aria-label',
		`${item.text} section, ${isExpanded ? 'expanded' : 'collapsed'}`,
	)
	...
	button.onclick = (e: MouseEvent) => {
		...
		if (iconSpan)
			iconSpan.textContent = isNowExpanded ? headingOpen : headingClosed
	}
