...
export const initPanelFocus = (): void => {
    if (activeIndex === 0) {
        const li = cols[0]?.querySelector<HTMLLIElement>('.active-file-row') ||
            cols[0]?.querySelector<HTMLLIElement>('li')
        if (li) highlightItem(li)
    }

    if (activeIndex === 2) {
        const li = cols[2]?.querySelector<HTMLLIElement>('li.keyboard-focus') ||
...
const navigatePanel = (dir: 'up' | 'down'): void => {
    ...
    const i = lastFocusedItem && items.includes(lastFocusedItem as HTMLLIElement)
        ? items.indexOf(lastFocusedItem as HTMLLIElement)
        : -1

    const ni = dir === 'down' ? (i + 1) % items.length : (i - 1 + items.length) % items.length
...
export const handleKeyDown = (e: KeyboardEvent): void => {
    ...
    if (isLeft) {
        ...
        const li = (activeEl.closest('li') as HTMLLIElement) || lastFocusedItem || null
