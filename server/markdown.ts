import { Marked, TokenizerAndRendererExtension, Tokens } from 'npm:marked@16'
import { markedSmartypantsLite } from 'npm:marked-smartypants-lite@1'
import { markedHighlight } from 'npm:marked-highlight@2'
import markedFootnote from 'npm:marked-footnote@1.4'
import hljs from 'npm:highlight.js@11'
import * as sanitizeHtml from 'npm:sanitize-html@2'
...
const preprocessMarkdown = (
	...
	if (addTitle && !markdown.startsWith('# '))
		markdown = `# ${toTitleCase(basename(currentPath).replace(/\.md$/, ''))}\n\n${markdown}`
	...
	processedMarkdown = processedMarkdown.replace(
		...
		(_match, lang, code) => {
			if (lang.startsWith('#!')) { //handle shebang
				lang = lang.slice(2).split(/[\/\\]/gm).pop().replace(/^env\s/, '').replace(
					/\-\-?[^\s]+/,
					'',
				).split(' ')[0]
			}
...
export const createCustomLinksExtension = (
	...
	const extension: TokenizerAndRendererExtension = {
		...
		tokenizer(src: string): Tokens.Generic | undefined {
			...
			if (match) {
				const [raw, doubleBracketTerm, bracketTerm, parenthesesTerm] = match
		...
		renderer(token: Tokens.Generic): string {
			...
			if (isWikilink) {
				...
				if (term.includes('.')) {
					...
					targetPathFromContentDir = findFilePath(vaultMap, term, '.md') ||
