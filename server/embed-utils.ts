...
export const extractEmbedProperties = (s: string): types.EmbedProperties => {
  ...
  for (const term of terms) {
    ...
    if (!value) {
		if (key === "numbered") { properties.numbered = true; continue }
		if (key === "notitle") { properties.title = ""; continue }
		if (/\d+x\d+/.test(key)) {
		...
		if (s.includes("#" + key)) { properties.startHeader = key; continue }
		if (s.includes("|" + key)) { properties.title = key; continue }
    }

    switch (key) {
		case "height": properties.height = value; continue
		case "width": properties.width = value; continue
		case "title": properties.title = stripQuotes(value); continue
		case "style": case "css": properties.css = value; continue
...
const processHeaderSection = (
	...
	if (properties.endHeader) {
		...
		const nextMatch = embeddedHTML.slice(startIndex + match[0].length).match(nextHeaderRegex)
