...
export const resolveFileRequest = async (
	...
	else absolutePath = join(Deno.cwd(), config.paths.contentDir, path.replace(/^\/\!/g, ''))
...
const determineRenderingType = (filePath: string): string | undefined => {
	...
	if (['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg', '.bmp', '.ico', '.apng'].includes(ext)) return 'image'
...
const renderDownloadContent = async (
	...
	try {
		...
		return `<div class="flex flex-col p-4 items-center text-center">
			...
			<p class="mt-1">File size: <strong class="font-bold">${formatBytes(
				fileInfo.size
			)}</strong></p>
...
export const loadFileToHTML_ = async (
	...
	if (!renderingType) {
		renderingType = detectBinaryContent(textContent)
			? 'download'
			: 'codeblock'
	}
