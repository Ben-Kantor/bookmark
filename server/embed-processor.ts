...
import {
  addLineNumbers,
  createEmbedWithProperties,
  extractEmbedProperties,
  splitHtmlWrappers,
} from "./embed-utils.ts";
import { escapeHTML, fileExists, toHTTPLink, warn } from "./lib.ts";
...
export const processEmbed = async (
  ...
  const targetPathFromContentDir = resolveTargetPath(
    doubleBracketTerm,
    rawLinkTarget,
    currentPath,
  );
...
const processHttpEmbed = async (
  ...
  if (hostname.includes("youtube.com") || hostname.includes("youtu.be")) {
    ...
    }" src="https://www.youtube.com/embed/${videoId}" frameborder="0" style="padding:0px" allowfullscreen></iframe>`;
  ...
  if (hostname.includes("vimeo.com")) {
    ...
    const iframe = `<iframe title="${
      escapeHTML(displayText)
    }" src="https://player.vimeo.com/video/${videoId}" frameborder="0" style="padding:0px" allowfullscreen></iframe>`;
  ...
  try {
    ...
    if (majorType === "video") {
      const video = `<video controls title="${
        escapeHTML(displayText)
      }" style="padding:0px">
    ...
    if (majorType === "audio") {
      const audio = `<audio controls title="${
        escapeHTML(displayText)
      }" style="padding:0px">
    ...
    if (mime === "application/pdf") {
      const iframe = `<iframe title="${
        escapeHTML(displayText)
      }" src="${httpLink}" type="application/pdf" style="padding:0px"></iframe>`;
      return createEmbedWithProperties(iframe, displayText, rawProperties);
    }

    if (
      mime === "text/markdown" || mime === "text/x-markdown" || ext === ".md" ||
      ext === ".markdown"
    ) {
    ...
    if (mime === "text/html") {
      const iframe = `<iframe title="${
        escapeHTML(displayText)
      }" src="${httpLink}" frameborder="0" style="padding:0px"></iframe>`;
      return createEmbedWithProperties(iframe, displayText, rawProperties);
    }
    ...
    try {
      ...
      const codeblock = await renderMarkdown(
        "```" + ext.slice(1) + "\n" + textContent + "\n```",
        httpLink,
        false,
        false,
      ...
      return createEmbedWithProperties(
        httpLink,
...
const processFileEmbed = async (
  ...
  try {
    const fullPathToEmbed = !httpLink
      ? join(contentDir, targetPathFromContentDir || "")
      : "";
    const properties = extractEmbedProperties(linkSource);
